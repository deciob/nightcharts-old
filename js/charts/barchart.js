(function() {
  define(["d3", "lodash"], function(d3, _) {
    ...
    return barChart = function(config) {
      var chart, color, events, format, height, internals, margin, width, x0Scale, x1Scale, xAxis, yAxis, yScale;
      ...
      internals.drawLegend = function(selection, data) {
      ...
      */
      ...
      internals.updateLegend = function(d, i, entering) {
        var item, name;
      ...
      internals.updateBars = function(d, i, entering) {
      ...
      internals.wireEvents = function() {
        return _.each(events, function(evt) {
          dispatch.on("" + evt + ".legend", interals.updateLegend);
          return dispatch.on("" + evt + ".onHover.bar", interals.updateBars);
        });
      };
      internals.extractBarNames = function(data) {
        return _.map(data[0].values, function(d) {
          return d.name;
        });
      };
      /*
      ...
      */
      ...
      chart = function(selection) {
        var bar_names, chart_group, data, dispatch, g, gEnter, svg;
        ...
        return internals.drawLegend(g, bar_names);
      };
